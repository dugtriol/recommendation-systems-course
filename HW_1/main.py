...
def main(input_path: str, output_path: str):
    ...
    # Используем все взаимодействия как веса
    # Учитываем лайки, дизлайки, время просмотра и другие действия
    interaction_weights = (
        train["timespent"].clip(lower=1).astype("float32") * 0.3 +
        train["like"].astype("float32") * 2.0 +
        (~train["dislike"]).astype("float32") * 0.5 +
        train["share"].astype("float32") * 3.0 +
        train["bookmark"].astype("float32") * 2.5 +
        train["click_on_author"].astype("float32") * 1.5 +
        train["open_comments"].astype("float32") * 1.2
    )
    
    data = interaction_weights.astype("float32")
    ...
    # Обучаем ALS-модель из implicit
    ...
    # Транспонируем матрицу для обучения (items x users)
    ...
    for u_idx in range(n_users):
        ...
        # Индексы айтемов с которыми уже были взаимодействия
        start = indptr[u_idx]
        end = indptr[u_idx + 1]
